...
const Completed = () => {
    ...
    const fetch = async () => {
        await axios
            .get("http://localhost:3001/complete")
            .then((res) => {
                setData(res.data)
            })
    ...
    const submit = async event => {
        ...
        formData.append("completed-image", file)
        formData.append("description", description)
        const result = await axios.post('http://localhost:3001/completed', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
        console.log(result)
    ...
    const handleDelete = async (id) => {
        const deleted = await axios.delete("http://localhost:3001/completed/" + id)
    }
    return (
        <div className='flex App'>
            ...
            <div className='w-full'>
                ...
                <div className="bg-white shadow-md rounded  px-8 pt-6 pb-8 mb-4 mt-10 w-2/4 flex justify-center flex-col m-auto ">
                    <span className='font-bold font-italic  text-3xl flex items-center justify-center animate-pulse'>Completed Projects</span>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            {data.length > 0 && data.map((singleData) => {
                                ...
                                const h = "http://localhost:3001/complete/" + singleData.name
